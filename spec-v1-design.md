...
- The prototype pre-loads with the modal already open at the **results step** — `mystore.com` is used as the sample URL so the design mode options are visible immediately on load
...
- Prototype enters directly at the results step; URL entry and scanning steps are accessible by clicking the input and retyping
